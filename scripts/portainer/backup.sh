...
# database port
if [ -z "${DB_PORT}" ]; then
  echo "DB_PORT not provided, defaulting to 27017"
  DB_PORT=9000
fi
...
if [[ -n "$DB_RESTORE_TARGET" ]]; then
  ...
  if [[ -f "$TMPRESTORE" ]]; then
    ...
    cat $workdir/* | mongodump --gzip --archive=country.archive --host $DB_SERVER --port $DB_PORT -u $DBUSER -p $DBPASS
    rm -rf $workdir
    /bin/rm -f $TMPRESTORE
  else
